...
/*
 * @notice This library is modified from https://github.com/code-423n4/2024-03-taiko/blob/main/packages/protocol/contracts/thirdparty/optimism/trie/MerkleTrie.sol
 * @dev The main modification is the addition of a new `tryGet` function and a new `GetResult` enum and `isStorageEmpty` function
 */
...
library MerkleTrie {
    ...
    /*
     * @notice Enum representing the result of a trie lookup.
     */
    enum GetResult {
        Exists, // Key exists, value is returned
        NotFound, // Key does not exist, proof is valid
        InvalidProof // Proof is invalid
    }

    /*
     * @notice Determines if the storage is empty for a given key.
     * @param _key   Key to check, as hex bytes.
     * @param _proof Merkle trie proof for the key.
     * @param _root  Known root of the Merkle trie.
     * @return isEmpty_ True if the storage is empty (key does not exist), false otherwise.
     */
    function isStorageSlotEmpty(
        bytes memory _key,
        bytes[] memory _proof,
        bytes32 _root
    ) internal pure returns (bool isEmpty_) {
        (GetResult result, ) = tryGet(_key, _proof, _root);
        if (result == GetResult.NotFound) {
            isEmpty_ = true; // Storage is empty (key does not exist)
        } else if (result == GetResult.Exists) {
            isEmpty_ = false; // Storage is not empty (key exists)
        } else {
            // For InvalidProof, we revert to prevent abuse
            revert("MerkleTrie: invalid proof provided");
        }
    }

    /*
     * @notice Attempts to retrieve the value associated with a given key without reverting.
     * @param _key   Key to search for, as hex bytes.
     * @param _proof Merkle trie proof for the key.
     * @param _root  Known root of the Merkle trie.
     * @return result Result enum indicating the outcome.
     * @return value_ Value of the key if it exists.
     */
    function tryGet(
        bytes memory _key,
        bytes[] memory _proof,
        bytes32 _root
    ) internal pure returns (GetResult result, bytes memory value_) {
        if (_key.length == 0) {
        ...
        for (uint256 i = 0; i < proof.length; i++) {
            ...
            if (currentKeyIndex > key.length) {
                // Key index exceeds total key length
                return (GetResult.InvalidProof, bytes(""));
            }
            ...
            if (currentNode.decoded.length == BRANCH_NODE_LENGTH) {
                if (currentKeyIndex == key.length) {
                    // At the end of the key, check for value in the branch node
                    value_ = RLPReader.readBytes(currentNode.decoded[TREE_RADIX]);
                    if (value_.length == 0) {
                    ...
                    // Extra proof elements are not allowed.
                    if (i != proof.length - 1) {
                        // Invalid proof, value node must be last
                        return (GetResult.InvalidProof, bytes(""));
                    }

                    return (GetResult.Exists, value_);
                } else {
                    // We're not at the end of the key yet.
                    uint8 branchKey = uint8(key[currentKeyIndex]);
                    RLPReader.RLPItem memory nextNode = currentNode.decoded[branchKey];
                    bytes memory nextNodeID = _getNodeID(nextNode);
                    if (nextNodeID.length == 0) {
                        // Next node does not exist, key does not exist
                        return (GetResult.NotFound, bytes(""));
                    }
                ...
                if (pathRemainder.length != sharedNibbleLength) {
                    // Path remainder does not match key remainder
                    return (GetResult.NotFound, bytes(""));
                }

                if (prefix == PREFIX_LEAF_EVEN || prefix == PREFIX_LEAF_ODD) {
                    if (keyRemainder.length != sharedNibbleLength) {
                        // Key remainder does not match path remainder exactly
                        return (GetResult.NotFound, bytes(""));
                    }
                    ...
                    // Extension node, move to the next node
                    currentNodeID = _getNodeID(currentNode.decoded[1]);
                    currentKeyIndex += sharedNibbleLength;
                } else {
                    // Unknown prefix
