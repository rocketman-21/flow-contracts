...
abstract contract VRGDACap is IVRGDACap {
    ...
    function xToY(
        ...
        if (avgTargetPrice <= 0) revert INVALID_TARGET_PRICE();

        return
            _pIntegral(timeSinceStart, sold + amount, avgTargetPrice) -
            _pIntegral(timeSinceStart, sold, avgTargetPrice);
    }

    // given # of tokens sold, returns integral of price p(x) = p0 * (1 - k)^(t - x/r)
    function _pIntegral(int256 timeSinceStart, int256 sold, int256 targetPrice) internal view returns (int256) {
        return
            wadDiv(
                -wadMul(
