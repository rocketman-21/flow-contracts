...
// using paperclip labs
const ownerMappingSlot = BigInt(3);
const tokenIds = [BigInt(788), BigInt(832)];
const ownerProofs = await Promise.all(tokenIds.map(async (tokenId) => {
...
const delegators: `0x${string}`[] = ['0xa2b6590a6dc916fe317dcab169a18a5b87a5c3d5'];
const delegateProofs = await Promise.all(delegators.map(async (delegator) => {
  const slot = keccak256(encodeAbiParameters([{type: 'address'}, {type: 'uint256'}], [delegator, delegateMappingSlot]));
